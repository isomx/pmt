...
function createItems(route, parents = []) {
  ...
  if (nestedItems) {
    resolvedNestedItems = nestedItems.map(route => createItems(route, parents.length ? [...parents, path] : [path]));
  }
